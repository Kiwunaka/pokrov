# POKROV 1.2.0 candidate 19 — известные ограничения
...
- Точный Windows setup прошёл isolated Windows 11 clean-app-state install,
  `11/11` file identity, обычный non-elevated UI, SCM `LocalSystem` service,
  protected-journal authenticated IPC, clean uninstall и migration с публичной
  1.1.6. Live TUN, connected DNS/egress, stop/restart, sleep/reboot/crash,
  connected uninstall, idle DNS/routes и интерактивный SmartScreen остаются
  отдельными проверками; clean-app-state VM не заявляется как clean OS.
- Candidate 18 отклонён и не должен публиковаться: обычный UI отвергал
  правильную службу `LocalSystem` из-за недоступного process token, после чего
  служба завершалась с access denied. Candidate 19 исправляет эту границу через
  SCM binding без ослабления ACL pipe и server-side caller authorization.
...
- Candidate 19 APK ещё не прошёл LDPlayer install/launch/byte-identity.
  LDPlayer находится за host Hiddify/TUN, поэтому даже после такого smoke его
  сетевой результат не будет доказательством AWG/DNS/egress.
- Физический ARM64-телефон должен проверить точный candidate 19 APK отдельно
...
- Smart DNS выключен по умолчанию. Live `dns.pokrov.space`, TLS/SNI и bounded
  ChatGPT/Gemini/Xbox policy доказаны на текущем runtime, но exact candidate 19
