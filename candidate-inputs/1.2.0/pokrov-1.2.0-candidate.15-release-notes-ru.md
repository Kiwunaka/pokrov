# POKROV 1.2.0 — direct beta candidate 15

Это новый точный закрытый кандидат Android и Windows после исправления
ограниченного по времени чтения managed profile. Он привязан к platform
`48669df4cfc466af6957ebfff7daf9eb5fb3cd66`, client
`75ba7e721cfee486f7189edd51de97aba2746722` и Core artifact source
`cd8f0f4169d570d693992a959d81d17c2c44884d`.

Candidate 15 не является публичным `v1.2.0`, не опубликован в магазинах и не
разрешает stable-продвижение без отдельного go/no-go.

## Что изменилось после candidate 14

Candidate 14 сохранён как неизменяемое evidence. Его exact APK успешно
устанавливался и запускался в LDPlayer, однако обновление managed profile могло
ждать несколько последовательных control-panel запросов и завершаться клиентским
таймаутом. Общий default-профиль воспроизводил ту же проблему, поэтому она не
была свойством AWG transport.

Platform successor ограничивает panel sync общим четырёхсекундным бюджетом,
выполняет независимые node-группы конкурентно, изолирует ошибку одной группы и
возвращает честный `pending_sync`, если фоновой источник не успел. Device-bound
AWG2/AWG 3.1/Hysteria2 профили не запускают не относящийся к ним legacy panel
read. Start-trial использует тот же ограниченный путь.

Исправление покрыто focused platform tests: timeout/cancellation, сохранение
успешного sync при отдельном runtime timeout, AWG lab без legacy panel,
конкурентность и изоляция node failure. Production deploy в рамках сборки
кандидата не выполнялся.

Client и Core не менялись. Поэтому шесть build `4049` артефактов переиспользованы
только после проверки имени, размера и SHA-256. Для candidate 15 заново
...
Platform fix не меняет transport engine или client bytes. Candidate 15 требует
собственный readback в LDPlayer: default, AWG 3.1, AWG2 и возврат в чистое
отключённое состояние. Старое runtime evidence не переносится автоматически.
